...
import { urlFor } from "@/sanity/lib/image";
import Image from "next/image";
import { useParams } from 'next/navigation'; // Import useParams to get the dynamic route params
...
const page = (): JSX.Element => {
  const params = useParams(); // Get the dynamic route params using useParams hook
  const [product, setProduct] = useState<Product | null>(null);

  useEffect(() => {
    const fetchProduct = async () => {
      if (!params.id) return; // Prevent fetching if params.id is undefined
    ...
    fetchProduct();
  }, [params.id]); // Dependency array watches params.id for changes
